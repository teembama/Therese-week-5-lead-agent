...
export function outreachRejectedEmbed(data: { runId: string; companyName: string; rejectedBy: string; reason: string; url: string | null }): DiscordEmbed {
  return {
    ...
    fields: [
      field("Company", data.companyName, true),
      field("Rejected by", data.rejectedBy, true),
...
export function outreachRegeneratedEmbed(data: { runId: string; companyName: string; regeneratedBy: string; direction: string; url: string | null }): DiscordEmbed {
  return {
    title: "Outreach regenerated",
    ...(data.url ? { url: data.url } : {}),
    color: COLOR_BLUE,
    fields: [
      field("Company", data.companyName, true),
      field("Regenerated by", data.regeneratedBy, true),
...
export async function notifyOutreachReview(data: {
  kind: "rejected" | "regenerated";
  runId: string;
  companyName: string;
  by: string;
  text: string;
  ...
  if (!webhookUrl()) return;
  const url = runUrl(data.runId, data.requestOrigin);
  await sendDiscordEmbed(
    data.kind === "rejected"
      ? outreachRejectedEmbed({ runId: data.runId, companyName: data.companyName, rejectedBy: data.by, reason: data.text, url })
      : outreachRegeneratedEmbed({ runId: data.runId, companyName: data.companyName, regeneratedBy: data.by, direction: data.text, url })
  );
}
